...
required Anthropic skill fields, length cap, and the When-to-use sections."""
...
def check_skill(path: Path) -> list[str]:
    ...
    if len(desc) < MIN_DESC_LEN:
        errs.append(f"{path}: description too short ({len(desc)} chars; need >= {MIN_DESC_LEN})")
    if not any(v in desc.lower() for v in ("use ", "invoke", "call ", "apply", "when ")):
...
if __name__ == "__main__":
    sys.exit(main(sys.argv[1:] or [".github/skills"]))
