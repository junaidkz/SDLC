...
## When NOT to use
- The diff is documentation-only (`docs/`, `README.md`) — no security surface to review.
- The diff is purely generated output (e.g. lock files, auto-formatted code) — the source change has already been reviewed.
- The user wants a code review for style / architecture — that's a different skill or general review.
