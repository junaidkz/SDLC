...
- The Reviewer needs the source of truth for what counts as a valid stamp (this skill is the reference).

## When NOT to use
- Pure formatting / whitespace changes with no semantic content — those don't need a stamp.
- Refactors with no observable behaviour change AND no test changes — they can roll up into the parent commit.
