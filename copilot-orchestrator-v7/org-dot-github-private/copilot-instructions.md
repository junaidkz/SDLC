...
- `instructions/security.instructions.md` — prompt-injection + vulnerability rules.
- `instructions/token-budget.instructions.md` — token-frugality rules.
...
- **Jira**: created during requirements authoring by Requirements Gatherer via task `jira:create-from-pending`, which runs `scripts/create_jira_from_feature.py`.
