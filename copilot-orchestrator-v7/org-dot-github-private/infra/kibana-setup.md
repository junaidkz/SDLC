...
| 4 | **Cost per REQ-ID** | Table | Sum of `cost_usd`, group: `req_ids.keyword` | last 30d. Requires enrichment — see Token enrichment below. |
...
- ⚠️ Token counts (`tokens_in`, `tokens_out`, `cost_usd`) — emitted as `0` in real time because VS Code Copilot doesn't expose them to custom agents. Back-filled by `scripts/enrich_audit_with_tokens.py`. See §8.
...
## 8. Token enrichment (post-hoc)

The agent can't see its own token counts in real time. `scripts/enrich_audit_with_tokens.py` back-fills them by joining audit events against an authoritative source:

| Source | Use when | Setup |
|---|---|---|
| **Local Copilot logs** | Per-developer dashboards on the dev machine | Zero — runs against `~/.config/github-copilot/logs/` and friends |
| **GitHub Copilot Enterprise usage API** | Org-wide cost rollups, central dashboard | `GITHUB_TOKEN` + `GITHUB_ORG` env vars |

Run as a scheduled job (cron / Task Scheduler / GitHub Action):
...
# Or daily centrally
python scripts/enrich_audit_with_tokens.py --source github --reindex --since "$(date -u -d '1 day ago' +%FT%TZ)"
```

The script is idempotent: events with `tokens_in > 0` are skipped on re-runs. The cost rate table is in the script — tune to your contract.

**Until enrichment runs at least once**, the "Cost per REQ-ID" and "Tokens in/out by model" tiles will show zeros. Latency, tool-call counts, rejection counts, and audit-trail replay all work without enrichment.
