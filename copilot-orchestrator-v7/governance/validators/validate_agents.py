...
def check_agent(path: Path) -> list[str]:
    ...
    if extras:
        # Warning, not error — Copilot evolves the spec
        print(f"{path}: warning — non-standard frontmatter fields: {sorted(extras)}", file=sys.stderr)
    desc = (data.get("description") or "").strip()
    if len(desc) < 20:
        errs.append(f"{path}: description too short ({len(desc)} chars; need >= 20 for reliable matching)")
    return errs
...
if __name__ == "__main__":
    sys.exit(main(sys.argv[1:] or ["agents"]))
